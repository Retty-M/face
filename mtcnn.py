...
#
#     frame = cv2.imread("./train_data/pic_others/other%d.jpg" % i)
#     img = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
#     img_color = to_rgb(img)
#
#     bounding_boxes, _ = detect_face.detect_face(img_color, minsize, pnet, rnet, onet, threshold, factor)
#     nrof_faces = bounding_boxes.shape[0]
#     print('Find faces: {}'.format(nrof_faces))
#
#     count = 0
#     for face_position in bounding_boxes:
#         face_position = face_position.astype(int)
...
#         cv2.rectangle(frame, (face_position[0], face_position[1]), (face_position[2], face_position[3]), (0, 255, 0), 2)
#         crop = img_color[face_position[1]:face_position[3], face_position[0]:face_position[2], ]
#
#         crop = cv2.resize(crop, (96, 96), interpolation=cv2.INTER_CUBIC)

        # print(crop.shape)
        # crop_faces.append(crop)
        # cv2.imwrite('./images/1-%d.png' % count, crop)
        # count += 1
        # plt.imshow(crop)
        # plt.show()
        # print i
